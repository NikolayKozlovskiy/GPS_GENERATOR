...
class User(ABC):
    def __init__(self, user_id: int, date_range, Network, profile_config) -> None:
        ...
        self.min_distance_h_w = profile_config['USER_PARAMS']['MIN_DISTANCE_H_W']
        self.min_distance_w_r = profile_config['USER_PARAMS']['MIN_DISTANCE_W_R']
        self.mean_velocity_ms = profile_config['USER_PARAMS']['MEAN_VELOCITY_MS']
        self.proximity_to_road = profile_config['USER_PARAMS']['PROXIMITY_TO_ROAD']
        self.transformer_to_WGS= Transformer.from_crs(Network.graph_crs, crs_4326, always_xy=True)
    ...
    def get_meaningful_locations(self, Network, gdf_hw, gdf_event, min_distance_h_w, min_distance_w_r): 
        ...
        while True: 
            ...
            home_node = Network.nodes.loc[gdf_hw.iloc[home_id]['nearest_node_id']]
            work_node = Network.nodes.loc[gdf_hw.iloc[work_id]['nearest_node_id']]
            ...
            if final_distance >= min_distance_h_w: # meters
                ...
                while i <= number_of_regular_locations: 
                    ...
                    regular_node = Network.nodes.loc[gdf_event.iloc[regular_id]['nearest_node_id']]
    ...
    def get_info_about_loc(self, list_of_ids, gdf): 
        ...
        for loc_id in list_of_ids: 
            list_of_info.append([gdf.iloc[loc_id]['nearest_node_id'],gdf.iloc[loc_id]['center_x'], gdf.iloc[loc_id]['center_y']])
    ...
    def create_list_of_locations(self, day_of_week, home_id, work_id, regular_loc_array, gdf_hw, gdf_event):
        ...
        event_id_list = self.get_regular_or_random_loc(regular_loc_array, gdf_event, number_of_events) 

        list_of_locations_not_event = self.get_info_about_loc(list_of_ids, gdf_hw)
        list_of_locations_event = self.get_info_about_loc(event_id_list, gdf_event)
    ...
    def get_chaotic_point(self, point_start, point_end, radius_of_buffer, proximity_to_road):
        ...
        final_intersection = points_intersection.intersection(path_between_points.buffer(proximity_to_road))
    ...
    def get_moving_points(self, Network, start_node, end_node,  start_coords, end_coords, user_id, data_array,\
        ...
        route = ox.distance.shortest_path(Network.graph_proj, start_node, end_node, weight="length")
        ...
        for i in range(number_of_points):
            ...
            if i != number_of_points-1: 
                chaotic_point=self.get_chaotic_point(points[i], points[i+1], length_of_distance_m, proximity_to_road)
                ...
                endLon, endLat = self.transformer_to_WGS.transform(chaotic_point.x, chaotic_point.y)
...
class User_walk(User): 
    def __init__(self, user_id: int, date_range, Network, profile_config):
        super().__init__(user_id, date_range, Network, profile_config)
        self.profile = profile_config['PROFILE_NAME']

    def get_meaningful_locations(self):
        super().get_meaningful_locations(self.Network, self.gdf_hw, self.gdf_event, self.min_distance_h_w, self.min_distance_w_r)
